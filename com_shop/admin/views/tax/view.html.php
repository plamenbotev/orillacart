...
class shopViewTax extends view {
    ...
    public function display() {
        ...
        $bar->appendButton('Link', 'new', __('new', 'com_shop'), admin_url('admin.php?page=component_com_shop-tax&task=add_group'), false, true);
        $bar->appendButton('Standard', 'delete', __('delete', 'com_shop'), 'document.adminForm.submit()', false, true);


        Factory::getMainframe()->addCustomHeadTag('tax-display', "
        ...
        ");
        parent::display('groups_list');
    }

    public function rates_list() {

        $bar = toolbar::getInstance('toolbar', __('Rates list', 'com_shop'), 'tax');
        $bar->appendButton('Link', 'new', __('new', 'com_shop'), admin_url("admin.php?page=component_com_shop-tax&task=add_rate&tax_group_id=" . $this->tax_group_id), false, true);
        $bar->appendButton('Standard', 'delete', __('delete', 'com_shop'), 'document.adminForm.submit()', false, true);

        Factory::getMainframe()->addCustomHeadTag('tax-rates_list', "
        <script type='text/javascript'>
            jQuery(document).ready(function() {
                jQuery('#toggle').click(function(){
                    jQuery(\"INPUT[type='checkbox']\").attr('checked', jQuery('#toggle').is(':checked'));   
    ...
    public function group_form() {
        ...
        $bar->appendButton('Link', 'cancel', __('cancel', 'com_shop'), admin_url('admin.php?page=component_com_shop-tax'), false, true);
