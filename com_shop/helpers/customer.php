...
class customer {
    ...
    public function fill_values() {
        ...
        if (Request::getCmd('con', null) == 'cart' &&
                Request::getCmd('task', null) == 'checkout' &&
                !Framework::is_admin() &&
        ...
        foreach (array('billing', 'shipping') as $type) {
            ...
            if (isset($this->fields[$type]->{$type . "_state"})) {
                if (isset($this->fields[$type]->{$type . "_country"})) {
                    ...
                    $this->fields[$type]->{$type . "_state"}->set_country(Factory::getApplication('shop')->getParams()->get('shop_country'));
                }
            }
        }
    }
        ...
        foreach ((array) $format as $ff) {
            ...
            if (isset($oshipping->{$ff})) {
                if ($oshipping->{$ff} instanceof country) {
                    ...
                    $country = Factory::getApplication('shop')->getTable('country')->load_by_code($oshipping->{$ff}->get_value());
                    $formatted[] = strings::htmlentities($country->country_name);
                } elseif ($oshipping->{$ff} instanceof state) {

                    $state = Factory::getApplication('shop')->getTable('state')->load_by_code($oshipping->{$ff}->get_country(), $oshipping->{$ff}->get_value());
                    if ($state->pk()) {
        ...
        foreach ((array) $format as $ff) {
            ...
            if (isset($obilling->{$ff})) {
                if ($obilling->{$ff} instanceof country) {
                    ...
                    $country = Factory::getApplication('shop')->getTable('country')->load_by_code($obilling->{$ff}->get_value());
                    $formatted[] = strings::htmlentities($country->country_name);
                } elseif ($obilling->{$ff} instanceof state) {

                    $state = Factory::getApplication('shop')->getTable('state')->load_by_code($obilling->{$ff}->get_country(), $obilling->{$ff}->get_value());
                    if ($state->pk()) {
                        $formatted[] = strings::htmlentities($state->state_name);
