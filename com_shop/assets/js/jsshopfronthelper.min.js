...
this.recalc_params.fire(data);jQuery("#com-shop .product-attributes select.property").each(function(i){switch(this.nodeName.toLowerCase()){case"select":if(this.value)
...
var $this=this;this.req=jQuery.ajax({type:"post",url:ajaxurl+"?action=framework-ajax-front&component=shop&con=product&task=get_price&pid="+pid,data:data,success:function(res,text){if(typeof res['id']!="undefined"){$this.ID=res['id'];for(var key in res.block){jQuery("#product_"+key).html(res.block[key]);}
