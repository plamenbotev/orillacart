jQuery(function () {
	...
	jQuery("#add_product_modal").on('click', "button#add_product_to_order", function (event) {
		...
		jQuery.ajax({
			url : ajaxurl + '?action=framework-ajax-admin&component=shop&con=orders&task=save_product_to_order&oid=' + document.getElementById('post_ID').value + '&pid=' + selected_product,
			type : "post",
			dataType : "json",
	...
	jQuery("input#select_parent").autocomplete({
		...
		select : function (event, ui) {
			if (ui.item) {
				...
				.load(ajaxurl + '?action=framework-ajax-admin&component=shop&con=orders&task=add_product_to_order&oid=' + document.getElementById('post_ID').value + '&pid=' + ui.item.pid)
				.dialog()
				.dialog('open');
	...
	jQuery("input#select_user").autocomplete({
		source : function (request, response) {
			jQuery.ajax({
				url : ajaxurl + "?action=framework-ajax-admin&component=shop&con=orders&task=get_users_list",
				dataType : "json",
				data : {
					str : request.term
				},
